 ...
 * source: backend/src/proto/server/forms.proto
...
export class ObjectFilter extends pb_1.Message {
    #one_of_decls: number[][] = [];
    ...
    static fromObject(data: {
        ...
        const message = new ObjectFilter({});
        if (data.id != null) {
            message.id = data.id;
        }
        return message;
    ...
    static deserialize(bytes: Uint8Array | pb_1.BinaryReader): ObjectFilter {
        const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes), message = new ObjectFilter();
    ...
    static deserializeBinary(bytes: Uint8Array): ObjectFilter {
        return ObjectFilter.deserialize(bytes);
    }
}
...
export class FormSubmission extends pb_1.Message {
    ...
    get where() {
        return pb_1.Message.getWrapperField(this, ObjectFilter, 1) as ObjectFilter;
    }
    set where(value: ObjectFilter) {
    ...
    static fromObject(data: {
        where?: ReturnType<typeof ObjectFilter.prototype.toObject>;
        data?: ReturnType<typeof FormSubmissionData.prototype.toObject>[];
        sourceHash?: string;
    }): FormSubmission {
        const message = new FormSubmission({});
        if (data.where != null) {
            message.where = ObjectFilter.fromObject(data.where);
        }
        if (data.data != null) {
    ...
    static deserialize(bytes: Uint8Array | pb_1.BinaryReader): FormSubmission {
        ...
        while (reader.nextField()) {
            ...
            switch (reader.getFieldNumber()) {
                case 1:
                    reader.readMessage(message.where, () => message.where = ObjectFilter.deserialize(reader));
                    break;
                case 2:
...
export abstract class UnimplementedFormsService {
    static definition = {
        getForm: {
            ...
            requestSerialize: (message: ObjectFilter) => Buffer.from(message.serialize()),
            requestDeserialize: (bytes: Buffer) => ObjectFilter.deserialize(new Uint8Array(bytes)),
    ...
    abstract getForm(call: grpc_1.ServerUnaryCall<ObjectFilter, Form>, callback: grpc_1.sendUnaryData<Form>): void;
...
export class FormsClient extends grpc_1.makeGenericClientConstructor(UnimplementedFormsService.definition, "Forms", {}) {
    ...
    getForm: GrpcPromiseServiceInterface<ObjectFilter, Form> = (message: ObjectFilter, metadata?: grpc_1.Metadata | grpc_1.CallOptions, options?: grpc_1.CallOptions): Promise<Form> => { if (!metadata) {
